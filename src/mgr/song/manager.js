class SongManager {
    constructor(aO, aN) {
        ...
        function Z(aO, aN) {
            aw();
            av = new vvw.song.Song();
            ax = false;
            ah();
        }
        function aw() {
            t("Generating ProgressPanel");
        ...
        function l(aN, aO, aR, aS) {
            ...
            t("Primary key of the song that is being saved after edit... " + aP);
            m = aN.name;
            K = aN.catIndex;
            x = aN.font;
            v = aN.font2;
        ...
        function O(aO, aQ) {
            ...
            if (!aQ) {
                ...
                aP.name = al.data[aO].name;
                aP.catIndex = al.data[aO].cat;
                aP.font = al.data[aO].font;
                aP.font2 = al.data[aO].font2;
                aP.timestamp = al.data[aO].timestamp;
                aP.yvideo = al.data[aO].yvideo;
                aP.bkgnd_fname = al.data[aO].bkgndfname;
                aP.key = al.data[aO].key;
                aP.copyright = al.data[aO].copy;
                aP.notes = al.data[aO].notes;
                aP.slides = D(al.data[aO].lyrics);
                var aN = al.data[aO].lyrics2;
                if (aN != null) {
                    aP.slides2 = D(aN);
                } else {
                    aP.slides2 = new Array();
                ...
                aP.slides2 = j(aP.slides, aP.slides2);
                aP.name2 = al.data[aO].title2;
                aP.tags = al.data[aO].tags;
                aP.slideseq = al.data[aO].slideseq;
                aP.rating = al.data[aO].rating;
                aP.chordsavailable = al.data[aO].chordsavailable;
                aP.usagecount = al.data[aO].usagecount;
                aP.subcat = al.data[aO].subcat;
            }
            return aP;
        }
        function A(aN) {
            ab = aN;
            t("In getSongsFromCat function " + aN);
        ...
        function e() {
            var aN = new vvw.song.Importer();
        }
        function i() {
            var aN = new vvw.song.PortXML(N, null, null, 1);
            aN.importXML();
        }
        function h() {
            var aN = new vvw.song.PortXML(N, null, null, 1);
        ...
        function Y() {
            var aN = new vvw.song.PortXML(N, ab, null, 2);
            aN.exportByCat();
        }
        function o(aN) {
        ...
        function ah() {
            aE = new air.SQLConnection();
            aE.addEventListener(air.SQLEvent.OPEN, z);
            aE.addEventListener(air.SQLErrorEvent.ERROR, I);
            var aN = air.File.applicationStorageDirectory.resolvePath(ad);
            aE.openAsync(aN);
        }
        function z(aN) {
            t("DB was created successfully");
            ax = true;
            aG();
        }
        function I(aN) {
        ...
        function aI(aO, aQ) {
            t("Updating the Song database TABLE: " + aO + " " + aQ);
            ...
            aS.execute();
            function aP() {
                t("Updating table worked !!!...");
            }
            function aN() {
                t("Failed updating table...");
                t("Error message:" + event.error.message);
                t("Details in creating table :" + event.error.details);
            }
        }
        function aG() {
            t(" Creating song table...");
            aA = new air.SQLStatement();
            aA.sqlConnection = aE;
            var aN =
                "CREATE TABLE IF NOT EXISTS sm (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, cat TEXT, font TEXT, font2 TEXT, timestamp TEXT, yvideo TEXT, bkgndfname TEXT, key TEXT, copy TEXT, notes TEXT, lyrics TEXT lyrics2 TEXT title2 TEXT tags TEXT slideseq TEXT rating INTEGER chordsavailable Boolean usagecount INTEGER subcat TEXT )";
            aA.text = aN;
        ...
        function r() {
            ...
            t("Notes Table created.....");
            songDBVersion = vvConfigObj.get_songDBVersion();
            t("Song DB version " + songDBVersion);
            if (songDBVersion < 2) {
                aI("title2", "TEXT");
                aI("tags", "TEXT");
                aI("slideseq", "TEXT");
        ...
        function M(aN) {
            ...
            aA.removeEventListener(air.SQLErrorEvent.ERROR, M);
            t("Error message:" + aN.error.message);
            t("Details in creating table :" + aN.error.details);
        }
        function aD() {
            ax = false;
        }
        ...
        function Q(aN) {
            ...
            function aQ(aS) {
                aO.removeEventListener(air.SQLEvent.RESULT, aP);
                aO.removeEventListener(air.SQLErrorEvent.ERROR, aQ);
                t("UPDATE error:" + aS.error);
                t("event.error.code:" + aS.error.code);
                t("event.error.message:" + aS.error.message);
            }
        }
        function at() {
            t("Getting ALL Data from Song DB");
            var aP = new air.SQLStatement();
            aP.sqlConnection = aE;
            ...
            aP.execute();
            function aO(aR) {
                t("Succesfuly got all data from Song DB");
                N = aP.getResult();
                a();
                scheduleObj.init();
            ...
            function aN(aR) {
                t("Song Manager data error...");
            }
        }
        function aC(aQ) {
            t("Getting ALL Titles from Song DB");
            var aP = new air.SQLStatement();
            aP.sqlConnection = aE;
        ...
        function C() {
            t("Getting ALL Categories from Song DB");
            var aP = new air.SQLStatement();
            aP.sqlConnection = aE;
            ...
            aP.execute();
            function aO(aR) {
                t("Succesfuly got all categories from Song DB");
                aq = aP.getResult();
                songNavObj.update_CategoryList(aq);
            }
            function aN(aR) {
                t("Song Manager data error while trying to get category...");
            }
        }
        function F() {
            t("Getting ALL Unique Fonts from Song DB");
            var aP = new air.SQLStatement();
            aP.sqlConnection = aE;
            ...
            aP.execute();
            function aO(aR) {
                t("Succesfuly got all fonts from Song DB");
                f = aP.getResult();
                d();
            }
            function aN(aR) {
                t("Song Manager data error while trying to get fonts...");
            }
        }
        function p(aP, aQ) {
            t("Searching Song DB " + aQ);
            var aR = new air.SQLStatement();
            aR.sqlConnection = aE;
            aR.addEventListener(air.SQLEvent.RESULT, aO);
            aR.addEventListener(air.SQLErrorEvent.ERROR, aN);
            var aT = "";
            if (aQ == SEARCH_TITLE) {
                aT = "SELECT * FROM sm WHERE name LIKE :param1 OR title2 LIKE :param1";
                aR.parameters[":param1"] = aP;
            }
            if (aQ == SEARCH_LYRICS) {
                aT =
                ...
                aR.parameters[":param1"] = aP;
                var aS = /%/gi;
                aR.parameters[":param2"] = aP.replace(aS, "");
            }
            if (aQ == SEARCH_TAGS) {
            ...
            aR.execute();
            function aO(aU) {
                aR.removeEventListener(air.SQLEvent.RESULT, aO);
                aR.removeEventListener(air.SQLErrorEvent.ERROR, aN);
                al = aR.getResult();
                if (aQ == SEARCH_TAGS && al.data == null) {
                    ...
                    songNavObj.searchComplete(al, aQ);
                }
            }
            function aN(aU) {
                aR.removeEventListener(air.SQLEvent.RESULT, aO);
                aR.removeEventListener(air.SQLErrorEvent.ERROR, aN);
                t("Song Manager search data error...");
                alert("Search function failed.");
            }
        }
        function X(aQ) {
            t("Deleting record with keyValue as primary key from Song Database...");
            var aP = new air.SQLStatement();
            aP.sqlConnection = aE;
            var aR = "";
            ...
            function aO(aS) {
                aP.removeEventListener(air.SQLEvent.RESULT, insertResult);
                aP.removeEventListener(air.SQLErrorEvent.ERROR, insertError);
                t("Error deleting song DB");
                t("event.error.code:" + aS.error.code);
                t("event.error.message:" + aS.error.message);
            }
        }
        function y(aP) {
            t("Deleting records based on category from Song Database...");
        ...
        function E() {
            t("Deleting Category for managed update from Song Database...");
            var aP = new air.SQLStatement();
            aP.sqlConnection = aE;
            var aQ = "";
            ...
            function aO(aR) {
                aP.removeEventListener(air.SQLEvent.RESULT, aN);
                aP.removeEventListener(air.SQLErrorEvent.ERROR, aO);
                t("Error deleting song DB category");
                t("event.error.code:" + aR.error.code);
                t("event.error.message:" + aR.error.message);
            }
        }
        function g() {
            t("about to copy new records...");
            var aN = "./song/default.db";
            var aP = air.File.applicationDirectory.resolvePath(aN);
            aE.addEventListener(air.SQLEvent.ATTACH, aQ);
            aE.addEventListener(air.SQLErrorEvent.ERROR, aO);
            aE.attach("newstuff", aP);
            function aQ(aR) {
                t("****** DB attach worked");
                aF();
            }
            function aO(aR) {
                t("Error message:" + aR.error.message);
                t("Details (Update Failed):" + aR.error.details);
            }
        }
        function aF() {
            t("Getting ALL Data from Original Song DB");
            var aP = new air.SQLStatement();
            aP.sqlConnection = aE;
            var aQ =
                "INSERT INTO sm SELECT NULL,name,cat,font,font2,timestamp,yvideo,bkgndfname,key,copy,notes,lyrics,lyrics2,title2,tags,slideseq,rating,chordsavailable,usagecount,subcat FROM newstuff.sm WHERE cat = :cat1 OR cat = :cat2 OR cat = :cat3";
            aP.text = aQ;
            aP.addEventListener(air.SQLEvent.RESULT, aO);
            aP.addEventListener(air.SQLErrorEvent.ERROR, aN);
            aP.parameters[":cat1"] = "VV Malayalam 2021";
        ...
        function aB() {
            ...
            function aR(aT, aS) {
                ...
                function aU(a0) {
                    aW.removeEventListener(air.SQLEvent.RESULT, aZ);
                    aW.removeEventListener(air.SQLErrorEvent.ERROR, aU);
                    t("UPDATE error:" + a0.error);
                    t("event.error.code:" + a0.error.code);
                    t("event.error.message:" + a0.error.message);
                    aX();
                }
                function aX() {
                    aQ++;
        ...
        function S() {
            ...
            function aS(aZ, aW, aV) {
                ...
                function aY(a2) {
                    aU.removeEventListener(air.SQLEvent.RESULT, aT);
                    aU.removeEventListener(air.SQLErrorEvent.ERROR, aY);
                    t("UPDATE error:" + a2.error);
                    t("event.error.code:" + a2.error.code);
                    t("event.error.message:" + a2.error.message);
                    aX();
                }
                function aX() {
                    aQ++;
...
vvw.provide('vvw.song').Manager = SongManager;